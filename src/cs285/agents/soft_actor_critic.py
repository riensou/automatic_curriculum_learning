...
class SoftActorCritic(nn.Module):
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        """
        return -1 * torch.mean(action_distribution.log_prob(action_distribution.rsample((self.num_actor_samples,))))
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        loss = -1 * torch.mean(log_probs * advantage)
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        loss.backward()
        gradient = [param.grad.view(-1) for param in self.actor.parameters()]
        gradient_norm = torch.norm(torch.cat(gradient))
        self.actor_optimizer.step()

        return {"actor_loss": loss.item(), "entropy": entropy.item(), "gradient_norm": gradient_norm.item(), "gradient": gradient}
    ...
    def update(
        ...
        if not torch.is_tensor(observations): observations = ptu.from_numpy(observations)
        if not torch.is_tensor(actions): actions = ptu.from_numpy(actions)
        if not torch.is_tensor(rewards): rewards = ptu.from_numpy(rewards)
        if not torch.is_tensor(next_observations): next_observations = ptu.from_numpy(next_observations)
        if not torch.is_tensor(dones): dones = ptu.from_numpy(dones)
        ...
        if self.target_update_period == None: # soft updates
            self.soft_update_target_critic(self.soft_target_update_rate)
        elif self.soft_target_update_rate == None and step % self.target_update_period == 0: # hard updates
            self.update_target_critic()
